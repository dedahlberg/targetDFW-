...
function normalizeTargetStoreId(storeId: string): string {
  const numericId = Number(storeId);
  return Number.isFinite(numericId) ? String(numericId) : storeId;
}
...
function buildRequestBody(tcin: string, visitorId: string) {
  ...
  const modules = contexts
    .filter((context) => wantedTypes.has(context.module_type ?? ''))
    .map((context) => ({
      module_placement_id: context.module_hierarchy?.module_placement_id ?? '',
      module_type: context.module_type ?? '',
      enrichment_context: encodeContext(context),
    }))
    .filter((module) => module.module_placement_id && module.module_type);

  return {
    page_context: Buffer.from(replacedText, 'utf8').toString('base64'),
    modules,
  };
}
...
async function resolveStoreLocation(store: (typeof stores)[number]): Promise<ResolvedLocation | null> {
...
export async function fetchTargetInventory(input: Input): Promise<InventoryResult> {
  ...
  try {
    ...
    const params = new URLSearchParams({
      ...
      scheduled_delivery_store_id: targetStoreId,
      scheduled_delivery_zip_code: location.zip,
    ...
    return {
      ...
      status: classify(result.quantity, result.availability),
    ...
    return {
      ...
      source: 'TARGET_PDP',
      fetchedAt,
      error: error instanceof Error ? error.message : 'Unknown Target PDP request error.',
    };
  }
}
